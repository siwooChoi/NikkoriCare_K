...
function iCareDataFromServer(){        // js, css 다하면 주석 풀어야됨
    ...
    function iCareGetChildrens(){
      ...
      $.ajax({
          url:"http://project-okyo.c9users.io/mobile/getDataforIcare.php",
...
var childDeviceinfo = function (name, mac, photo_address, uuid, major, minor, flat, flon, rssi, txpw, dist, scan_time){
  // 원아 기본 정보
  this.name = name;
  this.mac = mac;
...
var deviceInfoFromServer = function () {
    ...
    for (var i=0;i<theNumberOfChild;i++){
      ...
      eachChildDevice[i] = new childDeviceinfo(name,mac,photo_address,"","","","","",0,0,0);
    }
...
var powerToMeter = function () {
    ...
    var setRangeMeter = function () {
        ...
        for(var i = array_BLE_Meter.length-1 ; i >= 1 ; i--){
          ...
          "<option value="+i+">" + array_BLE_Meter[i] + "m 초과시 탐지</option>";
        }
      }
...
var buttonOn = function () {
  ...
  bluetoothle.enable();
  ms("textMessage", "블루투스 작동 시작 태스트");
}

var buttonOff = function () {
  bluetoothle.initialize();
  bluetoothle.disable();
  ms("textMessage","블루투스가 꺼졌습니다.");
}
      ...
      function (value){
        ...
        for (var i=0;i<eachChildDevice.length;i++) {
          if(eachChildDevice[i].mac == value.address){
            ...
            eachChildDevice[i].flat = getAduinoFlat(value);
            eachChildDevice[i].flon = getAduinoFlon(value);
...
var BT_stopScan = function () {
  ...
  backGroundAppPush();
    ms("textMessage","스캔하고 있지 않습니다.");
}

var BT_clear = function() {
    ms("textMessage","메시지 출력란");
    ms("scanMessage","스켄 메시지 출력란");
    ms("errorMessage","에러 메시지 출력란");
}
...
var enterBackGroundMode = function () {
  ...
  ms("backMessage","백 그라운드 모드 작동 시작");
}


var endBackGroundMode = function () {
  cordova.plugins.backgroundMode.disable();
  ms("backMessage","백 그라운드 모드 작동 중지");
}
...
var backGroundAlarm = function (num) {
  cordova.plugins.backgroundMode.setDefaults({
    ...
    title: "원아 이탈 감지",
    text: eachChildDevice[num].name +" 어린이를 확인해 주세요.",
...
var cdn = {}; // 좌표값 coordinate
   ...
   cdn.buttonGPS = function () {
     ms("textMessage","값을 불러오는 중입니다. 잠시 기다려 주세요.<br/>");
